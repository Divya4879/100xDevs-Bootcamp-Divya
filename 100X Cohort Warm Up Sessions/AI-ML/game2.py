...
class Deck:
    ...
    def shuffle(self): 
        if len(self.cards) > 0:  
            random.shuffle(self.cards)  # random.shuffle(sequence)

    def deal(self, number):   # no of cards chosen
        cards_chosen = []
        if len(self.cards) > 0:
            for i in range(number):
                cards_chosen.append(self.cards.pop())
        return cards_chosen
...
class Hand:
    ...
    # \ - line continues on in the following line
